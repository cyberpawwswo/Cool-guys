...
func set_dark_mode(dark: bool) -> void:
	_apply_muffle(dark)
	if floor_material:
		floor_material.albedo_color = Color(0, 0, 0, 1) if dark else _defaults.get("floor", Color.WHITE)
		floor_material.metallic_specular = 0.0 if dark else _defaults.get("floor_specular", 0.5)
	if wall_material:
		wall_material.albedo_color = Color(0, 0, 0, 1) if dark else _defaults.get("wall", Color.WHITE)
		wall_material.metallic_specular = 0.0 if dark else _defaults.get("wall_specular", 0.5)
	if inner_material:
		inner_material.albedo_color = Color(0, 0, 0, 1) if dark else _defaults.get("inner", Color.WHITE)
		inner_material.metallic_specular = 0.0 if dark else _defaults.get("inner_specular", 0.5)
	if sun:
		sun.light_energy = 0.0 if dark else _defaults.get("light", 1.0)
	if environment:
		...
		environment.ambient_light_energy = 0.0 if dark else _defaults.get("ambient_energy", 1.0)
		environment.ambient_light_color = Color(0, 0, 0, 1) if dark else _defaults.get("ambient_color", Color.WHITE)
		var sky: Sky = environment.sky
		if sky:
			var sky_mat := sky.sky_material as ProceduralSkyMaterial
			if sky_mat:
				sky_mat.sky_top_color = Color(0, 0, 0, 1) if dark else _defaults.get("sky_top", Color(0.4, 0.45, 0.55, 1))
				sky_mat.sky_horizon_color = Color(0, 0, 0, 1) if dark else _defaults.get("sky_horizon", Color(0.66, 0.67, 0.69, 1))
				sky_mat.ground_horizon_color = Color(0, 0, 0, 1) if dark else _defaults.get("ground_horizon", Color(0.66, 0.67, 0.69, 1))
				sky_mat.ground_bottom_color = Color(0, 0, 0, 1) if dark else _defaults.get("ground_bottom", Color(0.2, 0.17, 0.13, 1))
	var glow_manager := get_node_or_null("/root/GlowManager")
	if glow_manager and glow_manager.has_method("set_glow"):
		glow_manager.set_glow(dark)
